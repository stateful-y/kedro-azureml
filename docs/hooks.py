"""MkDocs hooks for the Kedro AzureML Pipeline documentation build.
...
def _build_api_table_html(project_root, prefix):
    ...
    for module_name, members in scans:
        module_label = _qualified_name(module_name, "").rstrip(".") or "kedro_azureml_pipeline"
...
def _resolve_see_also_url(name, prefix):
    ...
    """
    package = "kedro_azureml_pipeline"
...
def _resolve_member_identifier(name):
    ...
    ``OtherClass.build`` becomes ``kedro_azureml_pipeline.module.OtherClass.build``
...
def _link_entry(name, title, colon, entry, prefix):
    ...
    if "." in name and name.split(".", 1)[0] != "kedro_azureml_pipeline":
